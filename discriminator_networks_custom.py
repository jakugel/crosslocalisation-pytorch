...
class DiscriminatorLabelledCustom(nn.Module):

    def __init__(self, log_resolution, n_features=64, num_classes=10, num_layers=4,
        ...
        blocks = [DiscriminatorBlock(num_classes + 1, features[1], equalized, dropout)] + \
            [DiscriminatorBlock(features[i], features[i + 1], equalized, dropout) for i in range(1, n_blocks - 1)] + \
...
class DiscriminatorUnlabelledCustom(nn.Module):
    ...
    def __init__(self, log_resolution, n_features=64, num_layers=4, mdl_num_kernels=25, mdl_kernel_size=15,
        ...
        final_features1 = flattened_features + mdl_num_kernels
        final_features2 = flattened_features

        # two heads, one for unlabelled images, and one for cross-localised images (no MDL)
        if equalized:
            self.final1 = EqualizedLinear(final_features1, 1)
            self.final2 = EqualizedLinear(final_features2, 1)
        else:
            self.final1 = nn.Linear(final_features1, 1)
            self.final2 = nn.Linear(final_features2, 1)
