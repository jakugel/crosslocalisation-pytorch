...
class DiscriminatorLabelled(nn.Module):

    def __init__(self, log_resolution, n_features=64, max_features=256, num_classes=10, num_layers=4,
        ...
        if equalized:
            conv2d_l = EqualizedConv2d(num_classes + 1, n_features, 1)
        else:
            conv2d_l = nn.Conv2d(num_classes + 1, n_features, 1)
...
class DiscriminatorUnlabelled(nn.Module):
